...
public class ContextReductionServiceTests
{
    ...
    [Fact]
    public void ReduceIfNeeded_LargeConversation_ReducesToolBudgetDynamically()
    {
        // Large conversation that fills most of the budget, leaving little room for tools.
        // Even though RecentToolBudgetTokens is generous (200), the effective budget
        // should be reduced to the remaining room after conversation.
        var settings = new ContextReductionSettings
        {
            FallbackContextWindowSize = 500,
            SoftThresholdPercent = 0.85,
            RecentToolBudgetTokens = 200, // Generous — but won't all be usable
            MinReservedOutputTokens = 50,
            SafetyMargin = 1.0,
            CharsPerToken = 4.0,
        };
        ...
        // Add large conversation messages that consume most of the budget
        for (var i = 0; i < 10; i++)
        {
            messages.Add(new ChatMessage(ChatRole.User, new string('x', 100))
            {
                CreatedAt = baseTime.AddMinutes(i * 4),
            });
            messages.Add(new ChatMessage(ChatRole.Assistant, new string('y', 100))
            {
                CreatedAt = baseTime.AddMinutes(i * 4 + 1),
            });
        }
        ...
        Assert.True(result.Stage1Applied);
        Assert.True(result.RemovedMessageCount > 0,
            "Dynamic budget should force removal of older tool groups when conversation is large");
        AssertToolCallResultPairingIntegrity(result.Messages);
    }

    [Fact]
    public void ReduceIfNeeded_EffectiveToolBudgetNeverExceedsConfigured()
    {
        // When there's plenty of room, the effective tool budget should be capped
        // at the configured RecentToolBudgetTokens, not expand to fill available space.
        var settings = new ContextReductionSettings
        {
            FallbackContextWindowSize = 300,
            SoftThresholdPercent = 0.85,
            RecentToolBudgetTokens = 30, // Small configured budget
            MinReservedOutputTokens = 50,
            SafetyMargin = 1.0,
            CharsPerToken = 4.0,
        };
        ...
        {
            Assert.True(result.RemovedMessageCount > 0,
                "Configured budget cap should still limit tool groups even when room is available");
            AssertToolCallResultPairingIntegrity(result.Messages);
        }
    }
}
