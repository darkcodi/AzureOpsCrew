...
public class ChannelManagerPromptChunk : IPromptChunk
{
    public bool ShouldBeAdded(AgentRunData data)
    {
        ...
        var hasWorkerAgentsInChat = data.ParticipantAgents.Any(a => !string.Equals(a.Info.Username, "manager", StringComparison.InvariantCultureIgnoreCase));
        var isChannel = data.Channel != null;
        return isManagerAgent && hasWorkerAgentsInChat && isChannel;
    }
